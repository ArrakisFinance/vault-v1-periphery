...
// @notice External functions of this contract can only be called by ArrakisV1RouterWrapper
...
contract ArrakisV1Router is IArrakisV1Router {
    ...
    /// @notice addLiquidity adds liquidity to an ArrakisVaultV1 vault
        ...
        )
    {
        if (_removeData.receiveETH) {
            (amount0, amount1, liquidityBurned) = _removeData.vault.burn(
    ...
    /// @notice swapAndAddLiquidity makes a swap and deposits to an ArrakisVaultV1 vault and mints LP tokens
        ...
        )
    {
        ...
        (amount0, amount1, mintAmount) = _swapData.vault.getMintAmounts(
            amount0Use,
            amount1Use
        );
